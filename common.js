...
var Util = {
 ...
 increase: function(start, increment, max, maxLoops = 1) {
  var result = start + increment;
  var loops = 0;

  while (result >= max && loops < maxLoops) {
    result -= max;
    loops++;
  }

  while (result < 0 && loops < maxLoops) {
    result += max;
    loops++;
  }
...
var Game = {  
  ...
  run: function(options) {
    ...
    Game.loadImages(options.images, function(images) {
      ...
      options.ready(images); // tell caller to initialize itself because images are loaded and we're ready to go
      ...
      frame(); // yala beena nebda2 el hafla ! 
      Game.playMusic();
    });
  },
